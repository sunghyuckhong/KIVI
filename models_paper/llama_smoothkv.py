"""
Llama SmoothKV: calibrated channel smoothing + per-token INT4 KV cache.

Mirror of models/mistral_smoothkv.py for Llama.
"""
import math
...
class LlamaAttention_SmoothKV(LlamaAttention_KIVI):
    """Llama attention with SmoothKV: channel-smoothed per-token INT4 KV cache.

    Inherits paper-env LlamaAttention_KIVI's projections + rotary setup.
    Adds per-layer smoothing scales as buffers.
    """

    def __init__(self, config: LlamaConfig, s_K: torch.Tensor = None, s_V: torch.Tensor = None):
...
class LlamaDecoderLayer_SmoothKV(LlamaDecoderLayer_KIVI):
    def __init__(self, config, s_K=None, s_V=None):
        nn.Module.__init__(self)
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttention_SmoothKV(config=config, s_K=s_K, s_V=s_V)
        self.mlp = LlamaMLP(config)
        self.input_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)


class LlamaModel_SmoothKV(LlamaModel_KIVI):
    def __init__(self, config, calib=None):
        ...
        for i in range(config.num_hidden_layers):
            ...
            layers.append(LlamaDecoderLayer_SmoothKV(config, s_K=sK, s_V=sV))
        self.layers = nn.ModuleList(layers)


class LlamaForCausalLM_SmoothKV(LlamaForCausalLM_KIVI):
    _calib = None

    def __init__(self, config):
        LlamaPreTrainedModel.__init__(self, config)
        self.model = LlamaModel_SmoothKV(config, calib=self._calib)
