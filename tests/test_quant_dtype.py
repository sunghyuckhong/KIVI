...
def test_fake_quantize_v_pertoken_preserves_dtype(dtype):
    from vllm_custom.fake_quant_utils import fake_quantize_v_pertoken
...
def test_fake_quantize_k_pertoken_preserves_dtype(dtype):
    from vllm_custom.fake_quant_utils import fake_quantize_k_pertoken
    x = torch.randn(8, 4 * 128, dtype=dtype, device="cuda")
...
def test_fake_quantize_fp8_preserves_dtype(dtype):
    from vllm_custom.fake_quant_utils import fake_quantize_fp8
    x = torch.randn(8, 4 * 128, dtype=dtype, device="cuda")
...
def test_fp8_mse_well_below_int4(v_bf16):
    ...
    from vllm_custom.fake_quant_utils import fake_quantize_v_pertoken, fake_quantize_fp8
