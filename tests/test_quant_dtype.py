...
def test_fake_quantize_pertoken_preserves_dtype(dtype):
    from vllm.model_executor.layers.quantization.kv_fake_quant import fake_quantize_pertoken
    x = torch.randn(8, 4 * 128, dtype=dtype, device="cuda")
    out = fake_quantize_pertoken(x, num_kv_heads=4, head_dim=128, group_size=128, bits=4)
...
def test_int4_pertoken_mse_under_threshold(v_bf16):
    ...
    from vllm.model_executor.layers.quantization.kv_fake_quant import fake_quantize_pertoken
    out = fake_quantize_pertoken(v_bf16, num_kv_heads=4, head_dim=128, group_size=128, bits=4)
...
def test_fp8_mse_well_below_int4(v_bf16):
    ...
    from vllm.model_executor.layers.quantization.kv_fake_quant import fake_quantize_pertoken, fake_quantize_fp8
    out_fp8 = fake_quantize_fp8(v_bf16, 4, 128, group_size=128)
    out_int4 = fake_quantize_pertoken(v_bf16, 4, 128, 128, bits=4)
...
def test_fake_quantize_preserves_shape(layout):
    ...
    if layout == "2d":
        ...
        x = torch.randn(16, 4, 128, dtype=torch.bfloat16, device="cuda")
    out = fake_quantize_pertoken(x, 4, 128, 128, bits=4)
    assert out.shape == x.shape
