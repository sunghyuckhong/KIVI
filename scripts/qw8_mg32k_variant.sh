#!/bin/bash
# Run one variant of Qwen3-8B at MG=32k single-pass through 3 tasks sequentially.
# Args:
#   $1 = GPU (single index, e.g. "0")
#   $2 = method args (e.g. "--model bf16")
#   $3 = mtag (output filename method tag)
#   $4 = label (used for log filename)
set -uo pipefail
...
for task in gsm8k_32k minerva_math500 gpqa_main_cot_n_shot_32k; do
  ...
  $PY run_eval_vllm.py $METHOD_ARGS \
      ...
      --max_num_seqs 8 --batch_size 8 --tp 1 \
      --log_samples \
