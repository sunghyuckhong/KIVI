"""
Per-token KV cache quantization for Qwen2 — mirror of mistral_kivi_pertoken.py.
...
# Qwen2-2-7b has no GQA (num_kv_groups=1), so repeat_kv_quant is a no-op.
# Import the Mistral helper for correctness on GQA variants if ever used.
from models.mistral_kivi import repeat_kv_quant


class Qwen2Attention_KIVI_PerToken(nn.Module):
...
class Qwen2DecoderLayer_KIVI_PerToken(Qwen2DecoderLayer_KIVI):
    def __init__(self, config):
        nn.Module.__init__(self)
        self.hidden_size = config.hidden_size
        self.self_attn = Qwen2Attention_KIVI_PerToken(config=config)
        self.mlp = Qwen2MLP(config)
        self.input_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)


class Qwen2Model_KIVI_PerToken(Qwen2Model_KIVI):
    def __init__(self, config):
        ...
        self.layers = nn.ModuleList(
            [Qwen2DecoderLayer_KIVI_PerToken(config) for _ in range(config.num_hidden_layers)]
        )


class Qwen2ForCausalLM_KIVI_PerToken(Qwen2ForCausalLM_KIVI):
    def __init__(self, config):
        super(Qwen2PreTrainedModel, self).__init__(config)
        self.model = Qwen2Model_KIVI_PerToken(config)
